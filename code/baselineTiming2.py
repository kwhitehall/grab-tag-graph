...
def readData(userVariables):
    ...
    return unittestFile, graphVariables, TRMMCEdirName, CEdirName, filelist

def dataIngest(userVariables,unittestFile,filelist):
    #let's go!
    ...
    print ("-"*80)
    return mergImgs, timeList, LAT, LON, timeTaken
    
def findCloudElements(userVariables,unittestFile,graphVariables,LAT,LON,timeList,CEGraph,mergImgs,timeTaken):
    ...
    #CEGraph = mccSearch.find_cloud_elements(mergImgs,timeList,userVariables.DIRS['mainDirStr'], LAT,LON,userVariables,graphVariables)
    #findCEsEnd = time.time()
    ...
    #allCETRMMList = mccSearch.find_precip_rate(userVariables.DIRS['TRMMdirName'],timeList,userVariables,graphVariables)
    #findCETRMMEnd = time.time()
    #timeTaken += findCETRMMEnd - findCETRMMStart
...
def findCloudClusters(userVariables,unittestFile,graphVariables,CEGraph,prunedGraph,timeTaken):
...
def findMCC(userVariables,unittestFile,graphVariables,CEGraph,prunedGraph,MCCList,MCSList,timeTaken):
    print "\n -------------- TESTING findMCCs ----------"
